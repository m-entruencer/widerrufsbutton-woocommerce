...
- Umlaute in allen nutzer-sichtbaren Strings korrigiert (Admin-Liste, Detailansicht,
  Einstellungen, SetupNotice, Frontend-Bestätigung, Mail-Einstellungen in WC).

## 0.2.0
- Mailsystem von eigenem wp_mail() auf native WooCommerce-Mails (WC_Email) umgestellt.
  Vier Mail-Typen: Eingangsbestätigung, Akzeptanz, Ablehnung (Kunde) und neue
  Benachrichtigung an den Betreiber. Absender, Betreff, Texte und Layout laufen über
...
- Detail-Vorschau der Entwürfe jetzt im echten WC-Mail-Layout (iframe, kein Versand).
...
- Erste funktionale Implementierung (Skeleton-Stubs ausgefüllt):
  - Autoloader-Fallback in der Bootstrap-Datei (läuft ohne `composer install`).
  ...
  - Mailer: automatische Eingangsbestätigung, Entwurfs-Bodies und freigegebener
    Entscheidungsversand, Theme-überschreibbare Templates, Header-Filter
  ...
  - Mail-Templates auf echte Variablen umgestellt; uninstall.php löscht nur bei
    aktivem Setting.
- Entscheidung: keine automatische Entscheidung - auch Fall A wird Entwurf, nur die
  Eingangsbestätigung geht automatisch raus. Einbindung nur via Shortcode.
